...
V4.3.3 기준 A/B와 C의 주요 구조는 입력 무결성, 위험예산, 영속 청산상태, managed balance 격리까지 보강됐고, D는 **측정 → 저장 → 동일 의사결정 경로 재현**까지 왔다. 그러나 execution realism과 충분한 장기간 OOS가 아직 없으므로 “릴리즈가 성공했다”와 “수익성이 검증됐다”를 같은 뜻으로 사용하지 않는다.
