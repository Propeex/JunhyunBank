...
V4.3.0의 목적은 거래 횟수를 늘리거나 백테스트 수익률을 높이는 것이 아니다. 실제 계정에서 치명적일 수 있는 **잘못된 시장 데이터, 과도한 주문 크기, 느슨해지는 청산 기준, 잔고·주문 상태의 애매함**을 보수적으로 처리하는 것이 목적이다.
...
따라서 V4.3.0은 **실전 실패 형태를 더 안전하게 처리하는 릴리즈**이지, 자동매매가 수익을 낸다는 보증 또는 투자 권유가 아니다.
