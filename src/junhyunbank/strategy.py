...
class MicroFlowStrategy:
    ...
    def __init__(self, config: StrategyConfig) -> None:
        ...
        self._frames: dict[str, deque[SecondFrame]] = defaultdict(lambda: deque(maxlen=maxlen))
        self._current: dict[str, SecondFrame] = {}
        self._books: dict[str, BookSnapshot] = {}
        self._book_history: dict[str, deque[tuple[int, float, float]]] = defaultdict(lambda: deque(maxlen=maxlen))
    ...
    def on_trade(self, event: dict[str, Any]) -> None:
        ...
        if frame is None or frame.second != second:
            if frame is not None:
                self._frames[market].append(frame)
    ...
    def on_orderbook(self, event: dict[str, Any]) -> None:
        ...
        bid_depth = sum(p * s * w for p, s, w in zip(bid_prices, bid_sizes, weights))
        ask_depth = sum(p * s * w for p, s, w in zip(ask_prices, ask_sizes, weights))
        ...
        micro = (ask1 * bid_size1 + bid1 * ask_size1) / size_total if size_total else mid
        micro_bias = micro / mid - 1.0 if mid else 0.0
        self._books[market] = BookSnapshot(time.monotonic(), bid_prices, bid_sizes, ask_prices, ask_sizes, imbalance, micro_bias, max(0.0, spread_pct))
    ...
    def book_age(self, market: str) -> float:
        ...
        return max(0.0, time.monotonic() - book.received_at) if book else float("inf")

    def trade_age(self, market: str) -> float:
        received = self._last_trade_received.get(market)
        return max(0.0, time.monotonic() - received) if received is not None else float("inf")
    ...
    def warmup_ratio(self, market: str) -> float:
        return _clamp(len(self._series(market)) / max(1, self.config.min_warmup_seconds))
    ...
    def _feature_set(self, market: str) -> dict[str, float] | None:
        ...
        activity_q = _percentile_rank(_rolling_sums(values[:-1], aw), sum(values[-aw:]))
        ...
        aggression_history = [self._aggression(f) for f in frames[:-1] if f.trade_value > 0]
        aggression_q = _percentile_rank(aggression_history, aggression) if aggression > 0 else 0.0
        ...
        momentum_q = _percentile_rank(short_returns[:-1], short_now) if short_now > 0 and long_now > 0 else 0.0
        book = self._books.get(market)
        imbalance = micro_bias = 0.0
        spread_pct = 1.0
        book_q = 0.0
        if book is not None:
            imbalance, micro_bias, spread_pct = book.imbalance, book.micro_bias, book.spread_pct
            ...
            if len(history) >= 4:
                ...
                dh = [history[idx][1] - history[idx - 3][1] for idx in range(3, len(history) - 1)]
            dq = _percentile_rank(dh, delta) if delta > 0 else 0.0
            book_q = (max(1e-6, iq) * max(1e-6, mq) * max(1e-6, dq)) ** (1.0 / 3.0)
        quality = math.prod([max(1e-6, activity_q), max(1e-6, aggression_q), max(1e-6, book_q), max(1e-6, momentum_q)]) ** 0.25
        expected_returns = [abs(x) for x in self._returns(frames, self.config.expected_move_window_seconds) if math.isfinite(x)]
        expected_move = _quantile(expected_returns[-900:], 0.70) if expected_returns else 0.0
        one = self._returns(frames, 1)[-120:]
        sigma = statistics.pstdev(one) if len(one) >= 5 else 0.0
        return {"activity_q": activity_q, "aggression_q": aggression_q, "book_q": book_q, "momentum_q": momentum_q, "quality": quality, "aggression": aggression, "imbalance": imbalance, "micro_bias": micro_bias, "short_return": short_now, "long_return": long_now, "expected_move": expected_move, "realized_30s": sigma * math.sqrt(30.0), "spread_pct": spread_pct}

    def hot_score(self, market: str) -> float:
        f = self._feature_set(market)
        if not f:
            return 0.0
        return (max(1e-6, f["activity_q"]) * max(1e-6, f["aggression_q"]) * max(1e-6, f["momentum_q"])) ** (1.0 / 3.0) * 100.0

    def rank_markets(self, markets: list[str], limit: int) -> list[tuple[str, float]]:
    ...
    def market_regime(self, markets: list[str]) -> tuple[str, float]:
        ...
        btc_now = btc[-1] if btc else 0.0
        btc_scale = max(_quantile([abs(x) for x in btc[-600:]], 0.75) if btc else 0.0, 1e-6)
    ...
    def _is_pullback(self, market: str, expected_move: float) -> bool:
        ...
        if len(frames) < 65 or expected_move <= 0:
            return False
        recent = frames[-60:]
        low, peak, current = min(f.low for f in recent), max(f.high for f in recent), recent[-1].close
        if low <= 0 or peak <= low or current <= 0:
            return False
        advance, drawdown = peak / low - 1.0, peak / current - 1.0
        if advance < expected_move * 1.5 or not (expected_move * 0.15 <= drawdown <= expected_move * 0.85):
            return False
        short = self._returns(frames, 3)
        return bool(short and short[-1] > 0)

    def evaluate_entry(self, market: str, *, bid_fee: float, ask_fee: float, health: float, regime_factor: float) -> StrategyDecision:
        f = self._feature_set(market)
        if not f:
        ...
        quality, expected_move, spread = f["quality"], f["expected_move"], f["spread_pct"]
        cost = max(0.0, bid_fee) + max(0.0, ask_fee) + spread * 1.5
        if expected_move <= cost * 2.0:
            return StrategyDecision(Signal.HOLD, quality * 100.0, "예상 움직임이 거래비용 대비 부족", expected_move_pct=expected_move, round_trip_cost_pct=cost)
        kind = SignalKind.PULLBACK if self._is_pullback(market, expected_move) else SignalKind.IGNITION
        threshold = self.config.pullback_quality if kind == SignalKind.PULLBACK else self.config.ignition_quality
        if market in self._blocked_after_exit:
            if quality < 0.45:
                self._blocked_after_exit.discard(market)
            else:
                return StrategyDecision(Signal.HOLD, quality * 100.0, "이전 수급 신호가 아직 초기화되지 않음")
        if f["short_return"] <= 0 or f["long_return"] <= 0:
            return StrategyDecision(Signal.HOLD, quality * 100.0, "상승 모멘텀 미확인")
        r30 = self._returns(self._series(market), 30)
        if r30 and _percentile_rank(r30[:-1], r30[-1]) >= 0.99 and f["book_q"] < 0.80:
            return StrategyDecision(Signal.HOLD, quality * 100.0, "EXTENDED/EXHAUSTED")
        if quality < threshold:
            return StrategyDecision(Signal.HOLD, quality * 100.0, f"{kind.value} 품질 대기")
        edge = _clamp((expected_move - cost) / max(expected_move, 1e-9))
        fraction = _clamp(edge * quality * _clamp(health) * _clamp(regime_factor))
        risk = max(cost * 1.8, f["realized_30s"] * 1.25, spread * 3.0)
        one = [abs(x) for x in self._returns(self._series(market), 1)[-120:] if x != 0]
        typical = statistics.median(one) if one else expected_move / 60.0
        horizon = max(20.0, min(600.0, expected_move / max(typical, 1e-6)))
        reason = f"{kind.value}: Q={quality:.3f}, Activity={f['activity_q']:.2f}, Agg={f['aggression_q']:.2f}, Book={f['book_q']:.2f}, Mom={f['momentum_q']:.2f}"
        return StrategyDecision(Signal.BUY, quality * 100.0, reason, kind=kind, expected_move_pct=expected_move, round_trip_cost_pct=cost, initial_risk_pct=risk, capital_fraction=fraction, expected_horizon_seconds=horizon, hold_quality=quality)

    def suggest_emergency_risk(self, market: str, round_trip_cost_pct: float) -> float:
        f = self._feature_set(market)
        if not f:
            return max(round_trip_cost_pct * 2.0, 0.0)
        return max(round_trip_cost_pct * 1.8, f["realized_30s"] * 1.25, f["spread_pct"] * 3.0)

    def evaluate_position(self, market: str, *, entry_price: float, current_price: float, peak_price: float, initial_risk_pct: float, round_trip_cost_pct: float, elapsed_seconds: float, expected_horizon_seconds: float) -> StrategyDecision:
        ...
        if initial_risk_pct > 0 and pnl <= -initial_risk_pct:
            return StrategyDecision(Signal.SELL, 0.0, f"Emergency Stop {pnl:+.2%}")
        if not f:
            return StrategyDecision(Signal.HOLD, 0.0, "시장 데이터 워밍업/복구 중")
        parts = [max(1e-6, f["activity_q"]), max(1e-6, f["aggression_q"]), max(1e-6, f["book_q"]), max(1e-6, f["momentum_q"])]
        ...
        if self._weak_counts[market] >= 2:
            return StrategyDecision(Signal.SELL, hold * 100.0, f"수급 약화: HoldQ={hold:.3f}", hold_quality=hold)
        peak = max(peak_price, current_price, entry_price)
        mfe = peak / entry_price - 1.0
        if mfe >= round_trip_cost_pct * 2.5:
            trailing = max(f["realized_30s"] * 0.45, round_trip_cost_pct * 1.5, f["spread_pct"] * 2.0)
            if current_price <= peak * (1.0 - trailing):
                return StrategyDecision(Signal.SELL, hold * 100.0, f"Adaptive Trailing: MFE={mfe:+.2%}", hold_quality=hold)
        if expected_horizon_seconds > 0 and elapsed_seconds > expected_horizon_seconds * 1.5 and mfe < round_trip_cost_pct * 1.5 and hold < 0.60:
            return StrategyDecision(Signal.SELL, hold * 100.0, "기대 시간 내 모멘텀 미발생", hold_quality=hold)
        return StrategyDecision(Signal.HOLD, hold * 100.0, f"보유 유지: HoldQ={hold:.3f}", hold_quality=hold)
